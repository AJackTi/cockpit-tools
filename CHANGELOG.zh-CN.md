...
- **修复「官方登录」打开官方客户端后客户端立即关闭、提示「官方客户端已关闭，但没有检测到可导入的登录信息」**：注入授权地址拦截脚本时，`NODE_OPTIONS` 里的脚本路径没有做分隔符归一化，而 Node 解析 `NODE_OPTIONS` 时会把引号内的反斜杠当成转义符，Windows 路径被吃掉分隔符（`C:\Users\...` 变成 `C:Users...`），`--require` 找不到脚本，Electron 主进程在启动阶段直接退出（退出码 13），表现为客户端「刚打开就关闭」、读不到任何登录信息。现在注入路径统一换成正斜杠（Windows 同样接受）并保留引号以兼容含空格的路径，默认开启的「拦截浏览器跳转，直接显示授权地址」恢复可用；同时补上兜底：注入未能生效时自动退回官方原生流程再启动一次（仍可使用官方客户端自带的「复制登录链接」），不再直接判定本次登录失败。之前遇到该问题的用户把「拦截浏览器跳转」关掉即可绕过，现在默认开关可以保持开启。
- **修复商店版 Codex 更新后多开实例持续启动失败（拒绝访问 / os error 5）**：商店更新会换新的包目录，旧目录可能仍然存在但不允许执行，而已保存的路径只校验「文件是否存在」，因此不会被「路径不存在」的探测覆盖，每次启动都拿旧路径去执行、每次都被系统拒绝，必须手动「重置路径」才能恢复。现在直启被拒时会按当前用户已注册的商店包重新解析路径、写回配置并重试一次，成功即继续启动，不再要求用户手动重置；重试仍失败才降级到 PowerShell 兜底。探测顺序同时改为「当前注册的商店包优先于 WindowsApps 目录扫描」，避免扫描按目录名里的版本号挑中其它账户或已卸载包的残留目录。
- **修复 WindowsApps 直启被拒、改走 PowerShell 兜底时丢失附加环境变量**：该兜底启动分支此前只传 `CODEX_HOME` 与实例运行目录，临时登录注入用的 `NODE_OPTIONS` 等附加环境变量会被静默丢弃，导致注入在这类机器上不生效。现在附加环境变量会一并传给兜底分支。
- **修复商店版 Codex 更新后「客户端已经启动，Cockpit 却判定未运行」**：商店更新会换新的包目录，客户端也可能自行从新目录重启，而进程识别此前要求可执行文件**全路径完全相同**。路径不一致时读不到真实进程 PID，会回退到启动壳进程的 PID，随后立刻判定客户端已退出——多开实例表现成「启动后未匹配到实例 PID」，官方登录更会直接报「官方客户端已关闭，但没有检测到可导入的登录信息」，即便客户端其实好好地开着。现在商店包路径放宽为「同一个包族 + 同一个可执行文件名」即视为匹配（非商店路径仍要求完全一致），PowerShell 探测脚本与两处 sysinfo 兜底使用同一套规则。
- **修复经网关使用 DeepSeek 时工具调用与输出不相邻导致整轮被拒**：DeepSeek 按**位置**校验——每个工具调用的下一个项必须是它自己的输出，否则整段请求以 `No tool output found for tool call ...` 被拒，而被拒的这一轮会留在客户端历史里，线程此后无法继续，只能卸载再加载。Codex 在正常路径上就会破坏这个顺序：工具执行完的钩子会先把开发消息写进历史，可能早于工具输出落盘，于是出现「调用 → 消息 → 输出」。现在发往 DeepSeek 的请求会还原这层顺序（每个调用后面紧跟自己的输出，其余项保持相对顺序），本来就合法的历史是逐字节 no-op，不影响官方账号与其它上游。
- **修复回放给 DeepSeek 时推理正文丢失导致整段请求被拒**：网关响应出口为了让历史能切回官方账号，会把第三方推理正文从 `content` 迁移到 `summary` 并清空 `content`；DeepSeek 的思考模式反过来要求回放时必须带 `reasoning_text`，否则以 `The reasoning_text in the thinking mode must be passed back to the API` 拒绝整段请求。现在发往 DeepSeek 时会把这部分正文还原回 `reasoning_text`（同时保留 `summary`），两个方向各拿到自己需要的形状；只处理「`content` 为空、`summary` 有正文」的推理项，其它请求逐字节透传。

### 变更

- **商店版启动失败不再直接把内部错误串抛给用户**：`CODEX_MANAGED_STORE_LAUNCH_UNSAFE` 现在归类为可修复错误，切换到统一的 Windows 操作弹框，说明原因并提供「重新检测路径并重试」按钮（重探测路径后自动重试原操作）；详情区展示 `launch_path`、`launch_path_exists`、`registered_path`、`path_matches_registered`、`codex_home` 等诊断字段，复制错误时会一并带上，便于反馈定位。切换账号、API 服务、托盘与官方登录入口统一使用该提示，官方登录的失败信息也改为可读说明并保留原始原因。
- **经网关使用 DeepSeek 时固定串行执行工具调用**：并行批次正是「有调用没有输出」「调用与输出错位」这类损坏历史的来源（Codex 的 app-server 可能在下一次采样请求里抢先带上还没落盘的调用）。发往 DeepSeek 的请求现在会关闭并行工具调用，从源头避免该竞态；已经落盘的历史仍由上面的顺序还原与配对补齐兜底。
