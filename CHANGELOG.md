...
- **Fixed the Official login flow closing the official client right after launch with "the official client closed, but no importable sign-in data was found"**: when injecting the authorization-URL interception script the script path in `NODE_OPTIONS` was not normalized, and Node treats backslashes inside the quotes as escape characters, so a Windows path lost its separators (`C:\Users\...` became `C:Users...`). `--require` then failed to find the script and the Electron main process aborted during startup (exit code 13) — the client appeared to open and close instantly and no credentials could ever be read. The injected path now uses forward slashes (Windows accepts them too) while keeping the quotes so paths containing spaces still work, which restores the default **Intercept the browser jump and show the authorization URL here** switch. A fallback was added as well: when the injection does not take effect Cockpit relaunches the client through the official native flow instead of failing the sign-in outright (the client's own **Copy sign-in link** action remains available). Turning the interception switch off was the previous workaround; it is no longer needed.
- **Fixed multi-open instances failing to start permanently after a Store version update (access denied / os error 5)**: a Store update installs into a new package folder, and the old folder can still exist while no longer allowing execution. Because a saved path was only checked with "does this file exist", the detection that fixes missing paths never replaced it — every launch kept using the stale path and was rejected by Windows, and only a manual **Reset path** recovered it. Now, when a direct launch is denied, Cockpit resolves the path from the currently registered Store package, writes it back to the configuration and retries once, so no manual reset is needed; only if that retry also fails does it fall back to PowerShell. The detection order was also changed so the currently registered Store package is preferred over scanning the WindowsApps directory, which could otherwise pick a leftover folder belonging to another account or an uninstalled package purely by the version number in its name.
- **Fixed the PowerShell fallback dropping extra environment variables when a direct WindowsApps launch is denied**: that fallback branch previously passed only `CODEX_HOME` and the instance runtime directory, silently discarding extras such as the `NODE_OPTIONS` used for temporary-login injection, so the injection did nothing on such machines. The extra environment variables are now forwarded to the fallback branch.
- **Fixed "the client is running but Cockpit reports it as stopped" after a Store version update**: a Store update installs into a new package folder and the client can also restart itself from the new folder, but process detection required the executable **full path to match exactly**. When the paths differed Cockpit could not read the real PID and fell back to the launching shell process, then immediately decided the client had exited — multi-open instances logged "no instance PID matched after start", and Official login failed outright with "the official client closed, but no importable sign-in data was found" even though the client was running fine. Store package paths are now treated as the same instance when they share **the same package family and the same executable file name** (non-Store paths still require an exact match), and the PowerShell probe plus both sysinfo fallbacks use the same rule.
- **Fixed DeepSeek rejecting a whole turn because a tool call and its output were not adjacent**: DeepSeek validates **position** — the item directly after a tool call must be that call's own output, otherwise the request is rejected with `No tool output found for tool call ...` and the rejected turn stays in the client's history, wedging the thread until it is unloaded. Codex breaks that order on a normal path: its post-tool hook records a developer message before the tool output item lands, producing "call → message → output". Requests sent to DeepSeek now restore the order (each call is followed directly by its own output while every other item keeps its relative order), and history that is already valid is a byte-for-byte no-op, so official accounts and other upstreams are unaffected.
- **Fixed DeepSeek rejecting a whole request because replayed reasoning text was missing**: the gateway response path moves third-party reasoning text from `content` into `summary` and empties `content` so the history can be replayed to official accounts, but DeepSeek's thinking mode requires `reasoning_text` on every replayed reasoning item and otherwise rejects the request with `The reasoning_text in the thinking mode must be passed back to the API`. Requests sent to DeepSeek now restore that text into `reasoning_text` (keeping `summary` as well), so each direction gets the shape it accepts; only items with an empty `content` and a non-empty `summary` are touched, and every other request passes through byte-for-byte.

### Changed

- **Store launch failures no longer surface a raw internal error string**: `CODEX_MANAGED_STORE_LAUNCH_UNSAFE` is now classified as a repairable error and shown in the shared Windows operation dialog, which explains the cause and offers a **Re-detect path and retry** button (re-detects the path and then retries the original action). The details section lists `launch_path`, `launch_path_exists`, `registered_path`, `path_matches_registered` and `codex_home`, and copying the error includes them, which makes reports far easier to diagnose. Account switching, the API Service, the tray and the Official login flow all use this dialog, and the Official login failure message is now readable while keeping the original reason.
- **Tool calls are now serialized for DeepSeek through the gateway**: parallel batches are the source of the corrupted histories described above — Codex's app-server can start the next sampling request before the final call of a parallel batch has landed. Requests sent to DeepSeek now disable parallel tool calls so the race cannot happen, while already-corrupted history is still handled by the ordering and pairing repairs.
